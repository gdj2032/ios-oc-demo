...
@implementation LoadingViewConfigure

+ (instancetype)defaultConfigure {
    LoadingViewConfigure *configure = [[LoadingViewConfigure alloc] init];
...
///默认配置
@property (nonatomic, strong) LoadingViewConfigure *defaultConfigure;
///是否开始动画
...
#pragma mark -- 更新配置
- (void)updateWithConfigure:(void(^)(LoadingViewConfigure *configure))configBlock {
    if (configBlock) {
...
#pragma mark -- 默认配置
- (LoadingViewConfigure *) defaultConfigure {
    if (_defaultConfigure == nil) {
        _defaultConfigure = [LoadingViewConfigure defaultConfigure];
    }
