...
- (instancetype _Nonnull)initWithName:(NSString *)name
    ...
    GCDTimer *timer = [self initWithInterval:interval
                                     delaySecs:delaySecs
                                         queue:queue
                                       repeats:repeats
                                        action:action];
...
- (instancetype _Nonnull)initWithInterval:(NSTimeInterval)interval
    ...
    if (self = [super init]) {
        ...
        self.action = action;
        self.isRuning = NO;
        self.queue = dispatch_queue_create([[NSString stringWithFormat:@"GCDTimer.%p", self] cStringUsingEncoding:NSASCIIStringEncoding], DISPATCH_QUEUE_SERIAL);
...
+ (void)scheduledTimerWithName:( NSString * _Nonnull)name
    ...
    GCDTimer *timer = [self timer:string];
    if (timer) {
        return;
    }
    timer = [[GCDTimer alloc] initWithName:string
                                       interval:interval
                                      delaySecs:delaySecs
                                          queue:queue
                                        repeats:repeats
                                         action:action];
    [self setTimer:timer name:string];
}
#pragma mark -- 开始定时器
+ (void)start:(NSString *_Nonnull)name {
    __strong NSString *string = name;
    GCDTimer *GCDTimer = [self timer:string];
    if (GCDTimer) {
        [GCDTimer start];
    }
}
...
#pragma mark -- 获取定时器
+ (GCDTimer *_Nullable)timer:(NSString *_Nonnull)name {
    dispatch_semaphore_wait([GCDTimerManager sharedManager].semaphore, DISPATCH_TIME_FOREVER);
    __strong NSString *string = name;
    GCDTimer *GCDTimer = [[GCDTimerManager sharedManager].timerObjectCache objectForKey:string];
    dispatch_semaphore_signal([GCDTimerManager sharedManager].semaphore);
    return GCDTimer;
}
#pragma mark -- 储存定时器
+ (void)setTimer:(GCDTimer *_Nonnull)timer name:(NSString *_Nonnull)name {
    dispatch_semaphore_wait([GCDTimerManager sharedManager].semaphore, DISPATCH_TIME_FOREVER);
    [[GCDTimerManager sharedManager].timerObjectCache setObject:timer forKey:name];
    dispatch_semaphore_signal([GCDTimerManager sharedManager].semaphore);
}
#pragma mark -- 移除定时器
+ (void)removeTimer:(NSString *_Nonnull)name {
    dispatch_semaphore_wait([GCDTimerManager sharedManager].semaphore, DISPATCH_TIME_FOREVER);
    [[GCDTimerManager sharedManager].timerObjectCache removeObjectForKey:name];
    dispatch_semaphore_signal([GCDTimerManager sharedManager].semaphore);
}
@end
